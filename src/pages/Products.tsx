...
const Products = () => {
  ...
  const [quantitySort, setQuantitySort] = React.useState<SortOrder | null>(
    null,
  );
  ...
  const mergedProducts = React.useMemo(() => {
    ...
    const batchesByBarcode = batches.reduce<Record<string, InventoryBatch[]>>(
      (acc, batch) => {
    ...
    const updatedProducts = allProducts.map((product) => {
      ...
      if (productBatches && productBatches.length > 0) {
        ...
        const totalBatchQuantity = productBatches.reduce(
  ...
  React.useEffect(() => {
    ...
    if (savedDatabase) {
      try {
        ...
        console.log(
          "Loaded barcode database from localStorage:",
          parsedDatabase,
        );
      } catch (error) {
        console.error(
          "Error loading barcode database from localStorage:",
          error,
        );
      }
    }
  ...
  */
  ...
  const totalStockValue = React.useMemo(() => {
    return filteredProducts.reduce(
      (sum, p) =>
        sum + (typeof p.price === "number" ? p.price * p.quantity : 0),
      0,
    );
  }, [filteredProducts]);
  ...
  const totalStockValue2 = React.useMemo(
    () =>
      batches.reduce(
        (sum, b) =>
          sum +
          (typeof b.unit_price === "number" ? b.unit_price * b.quantity : 0),
        0,
      ),
    [batches],
  );
      ...
      <div className="mt-4 flex items-center justify-end">
        <div className="bg-muted/60 text-lg font-bold px-4 py-2 rounded shadow border">
          ...
          <span className="font-mono">
            {totalStockValue.toLocaleString("he-IL")}
          </span>{" "}
